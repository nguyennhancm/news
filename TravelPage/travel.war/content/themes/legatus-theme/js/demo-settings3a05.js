...
//    jQuery(".breaking-news .breaking-title, .breaking-news .breaking-controls, .main-content-right .panel h3, .content-panel .panel-header b, .page-pager > span, .page-pager > a:hover, .shortcode-content .spacer-break-2, .shortcode-content .spacer-break-3, .shortcode-content .spacer-break-4, .shortcode-content table thead, .shortcode-content .accordion > div > a, .content .filter a.active, .article-array li:before").css("background-color", newcolor);
    jQuery.ajax({
        ...
        type: 'GET',
        url: 'http://api.openweathermap.org/data/2.5/weather?id=1586203&units=metric&APPID=121d57459a447011991d913eb8a8fa6e',
        data: { get_param: 'value' },
        dataType: 'json',
        success: function (data) {
                jQuery(".weather-report").append("<span>THỜI TIẾT TẠI: </span>" +
                        "<b>Thành phố Cần Thơ</b>" +
                    "<img src='/content/themes/legatus-theme/images/" + data.weather[0].description + ".png' alt='" + data.weather[0].description + "' />" +
            "<font class='weather-meter' style='background: #e33f24;'>" + data.main.temp + " °C</font>"
                );
...
//    $.getJSON('http://api.openweathermap.org/data/2.5/weather?id=1566083&units=metric', { get_param: 'value' }, function(data) {
//        $.each(data, function(index, element) {
//            jQuery("weather-report").append("<span>Weather</span>" +
//                    "<b>Vietnam, Ho Chi Minh City</b>" +
//                    "<img src='wp-content/themes/legatus-theme/images/weather-rain.png' alt='Vietnam, Ho Chi Minh City' />" +
//                    "<font class='weather-meter' style='background: #e33f24;'>{element.name}°C</font>"
//            );
//        });
//    });
...
//	jQuery(".demo-settings").append("<a href='#show-settings' class='demo-button'><span class='icon-text'><i class='fa fa-gear'></i></span>Settings</a>");
...
//												"<a href='#' class='color-bulb active' style='background: url("+ot.imageUrl+"background-texture-1.jpg);'>&nbsp;</a>"+
...
//												"<a href='#' class='color-bulb' style='background-image: url("+ot.imageUrl+"background-photo-1.jpg);background-size: 100%; background-attachment: fixed; background-position: 50% 50%; background-repeat: repeat repeat;'>&nbsp;</a>"+
//												"<a href='#' class='color-bulb' style='background-image: url("+ot.imageUrl+"background-photo-2.jpg);background-size: 100%; background-attachment: fixed; background-position: 50% 50%; background-repeat: repeat repeat;'>&nbsp;</a>"+
//												"<a href='#' class='color-bulb' style='background-image: url("+ot.imageUrl+"background-photo-3.jpg);background-size: 100%; background-attachment: fixed; background-position: 50% 50%; background-repeat: repeat repeat;'>&nbsp;</a>"+
//												"<a href='#' class='color-bulb' style='background-image: url("+ot.imageUrl+"background-photo-4.jpg);background-size: 100%; background-attachment: fixed; background-position: 50% 50%; background-repeat: repeat repeat;'>&nbsp;</a>"+
//												"<a href='#' class='color-bulb' style='background-image: url("+ot.imageUrl+"background-photo-5.jpg);background-size: 100%; background-attachment: fixed; background-position: 50% 50%; background-repeat: repeat repeat;'>&nbsp;</a>"+
//											"</div>"+
//										"</div>"+
...
//			jQuery(".dat-menu-container").css("background", newcolor);
//		}
//
//		return false;
//	});
